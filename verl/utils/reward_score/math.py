...
import re
from deepscaler_.rewards.math_utils.utils import grade_answer_sympy, grade_answer_mathd
# from verl.trainer.ans_eval.gsm8k import *
...
def extract_leading_score(text: str):
    match = re.match(r'\s*(\d(?:\.\d+)?|\.\d+)', text)
    if match:
        return float(match.group(1))
    raise ValueError(f"Cannot extract a score from the text: {text}")

...
from verl.trainer.ppo.ray_trainer_w_sft import RayPPOTrainer_sft
...
class TaskRunner:
    def run(self, config):
        ...
        from verl.trainer.ppo.ray_trainer_w_sft import ResourcePoolManager, Role
        ...
        if config.data.attack_mode:
            train_dataset = create_rl_dataset(config.data.attack_train_files, config.data, tokenizer, processor)
        else:
            train_dataset = create_rl_dataset(config.data.train_files, config.data, tokenizer, processor)
        val_dataset = create_rl_dataset(config.data.val_files, config.data, tokenizer, processor)
        attack_val_dataset = create_rl_dataset(config.data.attack_val_files, config.data, tokenizer, processor)
        train_sampler = create_rl_sampler(config.data, train_dataset)

        # Initialize the PPO trainer.
        trainer = RayPPOTrainer_sft(
            config=config,
            tokenizer=tokenizer,
            judge=Judge_actor,
            processor=processor,
            ...
            attack_val_dataset=attack_val_dataset,
